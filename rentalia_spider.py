...
class Rentalia_Scraper:
  ...
  def __init__(self):
    ...
    self.driver = webdriver.Chrome(executable_path = r"C:\\Program Files\\chromedriver.exe", options = self.options) # Setting webdriver, path where driver is installed (driver version must be the same as browser version)
                                                                                                            # and the options.
  ...
  def Start_Rentalia_Driver(self): # Accessing to the main url and cookies
    ...
    self.driver.get(url) 
    cookies = self.driver.find_element(By.XPATH, '//*[@id="didomi-host"]/div/div/div/div//div[2]/button[2]') #Accept cookies button
    cookies.click()
  ...
  def Extract_Properties_To_CSV(self): # Scrapìng property interest fields
        ...
        headers = ['Título','Ubicación','Número referencia','Precio','Capacidad','Link','Teléfono móvil'] # First row of the document
        with open(f'rentalia.csv','w+', encoding = 'utf-8', newline='') as f:
          ...
          while i < 525: # Total pages
              for a in range(0, 15):  # Properties per page
                ...
                try:
                  capacity = self.driver.find_element(By.CLASS_NAME, 'characteristic').find_element(By.TAG_NAME, 'p').text
                ...
                line = [title, lctn, ref, price, capacity, link, tel] 
